  ...
  return (
    <div style={{ background: "#111820", border: "0.5px solid #1a2a3e", borderRadius: 14, padding: 18 }}>
      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
        <div style={{ width: 40, height: 40, borderRadius: 10, background: "#1a2a3e", animation: "pulse 1.5s ease-in-out infinite", flexShrink: 0 }}></div>
        <div style={{ flex: 1 }}>
...
export default function App() {
  ...
  const exportJobs = () => {
    ...
    a.href = url; a.download = "smartapply-jobs.txt"; a.click()
  ...
  const exportAnalysis = () => {
    ...
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a")
    a.href = url; a.download = "smartapply-career-analysis.txt"; a.click()
    URL.revokeObjectURL(url)
  }

  const resetCoach = () => { setFile(null); setJobRole(""); setAnalysis(null); setCoachStep(1) }

  const sidebarW = isMobile ? 0 : 200
  const mainPad = isMobile ? "20px 16px 80px" : "32px 36px"
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#090d12", fontFamily: "var(--font-sans)", color: "#d4dde8" }}>
      ...
      {!isMobile && (
        <div style={{ width: 200, background: "#0d1218", borderRight: "0.5px solid #1a2332", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 10 }}>
          <div style={{ padding: "16px 18px", borderBottom: "0.5px solid #1a2332", display: "flex", alignItems: "center", gap: 8 }}>
            <img src="/logo.png" width="32" height="32" alt="SmartApply logo" style={{ borderRadius: 6 }} />
          ...
          <div style={{ padding: "14px 0", flex: 1 }}>
            ...
            {([["jobs", "ti-search", "Job Finder"], ["coach", "ti-user-check", "Career Coach"]] as const).map(([id, icon, label]) => (
              <div key={id} onClick={() => setPage(id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 18px", fontSize: 13, cursor: "pointer", background: page === id ? "#0d2040" : "transparent", color: page === id ? "#378ADD" : "#667788", fontWeight: page === id ? 500 : 400, borderLeft: page === id ? "2px solid #378ADD" : "2px solid transparent" }}>
      ...
      {/* Mobile Top Bar */}
      {isMobile && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, height: 52, background: "#0d1218", borderBottom: "0.5px solid #1a2332", display: "flex", alignItems: "center", padding: "0 16px", gap: 10, zIndex: 10 }}>
          <img src="/logo.png" width="24" height="24" alt="SmartApply logo" style={{ borderRadius: 6 }} />
          <span style={{ fontSize: 15, fontWeight: 500, color: "#e2ecf4" }}>SmartApply</span>
          <span style={{ fontSize: 10, background: "#0d2040", color: "#378ADD", padding: "2px 7px", borderRadius: 20 }}>beta</span>
        </div>
      )}
      ...
      {/* Main content */}
      <div style={{ marginLeft: sidebarW, flex: 1, padding: mainPad, marginTop: isMobile ? 52 : 0 }}>
        ...
        {page === "jobs" && (
          <div>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: isMobile ? 18 : 22, fontWeight: 500, color: "#e2ecf4" }}>Find Jobs</div>
                <div style={{ fontSize: 12, color: "#445566", marginTop: 3 }}>AI searches the web for real, live job listings</div>
              </div>
              {(jobs.length > 0 || rawResult) && (
                <button onClick={exportJobs} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, padding: "7px 14px", borderRadius: 10, background: "#0d2040", color: "#378ADD", border: "0.5px solid #1a3a5e", cursor: "pointer" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: searchHistory.length > 0 ? 10 : 20 }}>
              <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10, background: "#0d1218", border: "0.5px solid #1a2a3e", borderRadius: 12, padding: "10px 14px" }}>
                <i className="ti ti-search" style={{ fontSize: 15, color: "#334455" }} aria-hidden="true"></i>
                <input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && searchJobs()}
                  placeholder={isMobile ? "Search jobs..." : "e.g. Computer Vision intern in Bangalore..."}
              ...
              <button onClick={() => searchJobs()} disabled={loadingJobs}
                style={{ background: "#378ADD", color: "white", border: "none", borderRadius: 12, padding: "10px 18px", fontSize: 13, fontWeight: 500, cursor: "pointer", opacity: loadingJobs ? 0.6 : 1, whiteSpace: "nowrap" }}>
                {loadingJobs ? "..." : "Search"}
              </button>
            </div>
            ...
            {searchHistory.length > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                ...
                {searchHistory.map((h, i) => (
                  <div key={i} onClick={() => searchJobs(h)} style={{ fontSize: 12, padding: "4px 12px", borderRadius: 20, background: "#0d1a2e", color: "#5a8ab8", border: "0.5px solid #1a2a3e", cursor: "pointer" }}>{h}</div>
                ))}
              </div>
            )}
            ...
            {loadingJobs && (
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 10, color: "#445566", fontSize: 13, marginBottom: 16 }}>
            ...
            {!loadingJobs && jobs.length > 0 && (
              <div>
                <div style={{ fontSize: 12, color: "#445566", marginBottom: 12 }}>{jobs.length} jobs found</div>
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
                  {jobs.map((job, i) => (
                    <div key={i}
                      style={{ background: "#0d1218", border: "0.5px solid #1a2332", borderRadius: 14, padding: 16, display: "flex", flexDirection: "column", gap: 10 }}
                      onMouseEnter={e => !isMobile && (e.currentTarget.style.borderColor = "#378ADD")}
                      onMouseLeave={e => !isMobile && (e.currentTarget.style.borderColor = "#1a2332")}
                    >
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                        <div style={{ width: 38, height: 38, borderRadius: 10, background: "#0d2040", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 500, color: "#378ADD", flexShrink: 0 }}>
                          {job.company ? job.company.slice(0, 2).toUpperCase() : "??"}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 14, fontWeight: 500, color: "#e2ecf4", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{job.title}</div>
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: "auto" }}>
                        {job.link && (
                          <a href={job.link} target="_blank" rel="noopener noreferrer"
                            style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 12, color: "white", background: "#378ADD", textDecoration: "none", fontWeight: 500, padding: "8px 0", borderRadius: 8 }}>
                            Apply now
                          </a>
                        )}
                        {job.link && (
                          <button onClick={() => copyLink(job.link, i)}
                            style={{ padding: "8px 12px", borderRadius: 8, background: "#0d2040", color: copiedLink === i ? "#5ab87a" : "#378ADD", border: "0.5px solid #1a3a5e", cursor: "pointer", fontSize: 12 }}>
                            {copiedLink === i ? "✓" : <i className="ti ti-copy" style={{ fontSize: 13 }} aria-hidden="true"></i>}
                          </button>
                        )}
                      </div>
                    </div>
            ...
            {!loadingJobs && rawResult && !jobs.length && searched && (
              <div style={{ background: "#0d1218", border: "0.5px solid #1a2332", borderRadius: 14, padding: 18 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, paddingBottom: 12, borderBottom: "0.5px solid #1a2332" }}>
                  <i className="ti ti-sparkles" style={{ fontSize: 14, color: "#378ADD" }} aria-hidden="true"></i>
        ...
        {page === "coach" && (
          <div>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: isMobile ? 18 : 22, fontWeight: 500, color: "#e2ecf4" }}>Career Coach</div>
                <div style={{ fontSize: 12, color: "#445566", marginTop: 3 }}>AI-powered resume analysis and roadmap</div>
              </div>
              {analysis && (
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={exportAnalysis} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, padding: "7px 14px", borderRadius: 10, background: "#0d2040", color: "#378ADD", border: "0.5px solid #1a3a5e", cursor: "pointer" }}>
                    <i className="ti ti-download" style={{ fontSize: 13 }} aria-hidden="true"></i>
                    {!isMobile && "Export"}
                  </button>
                  <button onClick={resetCoach} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, padding: "7px 14px", borderRadius: 10, background: "#0d1218", color: "#667788", border: "0.5px solid #1a2332", cursor: "pointer" }}>
                ...
                <div key={num} style={{ display: "flex", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <div style={{ width: 26, height: 26, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 500, background: coachStep > i + 1 ? "#378ADD" : coachStep === i + 1 ? "#0d2040" : "#0d1218", color: coachStep > i + 1 ? "white" : coachStep === i + 1 ? "#378ADD" : "#334455", border: coachStep === i + 1 ? "1.5px solid #378ADD" : coachStep > i + 1 ? "none" : "0.5px solid #1a2332" }}>
                      {coachStep > i + 1 ? <i className="ti ti-check" style={{ fontSize: 12 }} aria-hidden="true"></i> : num}
                    </div>
                    {!isMobile && <span style={{ fontSize: 12, color: coachStep === i + 1 ? "#378ADD" : "#334455" }}>{label}</span>}
                  </div>
                  {i < 2 && <div style={{ width: isMobile ? 24 : 40, height: 1, background: coachStep > i + 1 ? "#378ADD" : "#1a2332", margin: "0 8px" }}></div>}
                </div>
              ))}
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16 }}>
                <div style={{ background: "#0d1218", border: "0.5px solid #1a2332", borderRadius: 14, padding: 20 }}>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <label htmlFor="resume-upload" style={{ display: "block", border: "1.5px dashed #1a2a3e", borderRadius: 10, padding: "20px", textAlign: "center", cursor: "pointer" }}>
                      <i className="ti ti-upload" style={{ fontSize: 22, color: "#334455", display: "block", marginBottom: 8 }} aria-hidden="true"></i>
                      {file ? <span style={{ fontSize: 13, color: "#378ADD", fontWeight: 500 }}>{file.name}</span>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%", marginTop: 8 }}>
                      {["Skill gap analysis", "Match score", "Interview prep", "30/60/90 day roadmap"].map((f, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "#445566" }}>
                          <i className="ti ti-check" style={{ fontSize: 13, color: "#1a3a5e" }} aria-hidden="true"></i>{f}
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {coachStep === 2 && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 0", gap: 16 }}>
                <div style={{ width: 48, height: 48, border: "3px solid #0d2040", borderTopColor: "#378ADD", borderRadius: "50%", animation: "spin 0.9s linear infinite" }}></div>
                <div style={{ fontSize: 15, color: "#e2ecf4", fontWeight: 500 }}>Analyzing your resume...</div>
                <div style={{ fontSize: 13, color: "#445566", textAlign: "center" }}>Reading skills, comparing with {jobRole}</div>
              </div>
            )}
            ...
            {coachStep === 3 && analysis && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 2fr", gap: 14 }}>
                  <div style={{ background: "#0d1218", border: "0.5px solid #1a2332", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ fontSize: 11, color: "#445566", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>Match Score</div>
                    <div style={{ fontSize: isMobile ? 48 : 60, fontWeight: 500, color: "#378ADD", lineHeight: 1 }}>
                      <AnimatedScore target={analysis.match_score} /><span style={{ fontSize: 20, color: "#334455" }}>%</span>
                    </div>
      ...
      {/* Mobile Bottom Nav */}
      {isMobile && (
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, height: 60, background: "#0d1218", borderTop: "0.5px solid #1a2332", display: "flex", zIndex: 10 }}>
          {([["jobs", "ti-search", "Jobs"], ["coach", "ti-user-check", "Coach"]] as const).map(([id, icon, label]) => (
            <div key={id} onClick={() => setPage(id)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, cursor: "pointer", color: page === id ? "#378ADD" : "#445566" }}>
              <i className={`ti ${icon}`} style={{ fontSize: 20 }} aria-hidden="true"></i>
              <span style={{ fontSize: 11, fontWeight: page === id ? 500 : 400 }}>{label}</span>
            </div>
          ))}
        </div>
      )}
